...
-(NSString *)getDeformDescWithCitizenName2:(NSString *)citizenName{
    ...
    if (deformArray.count>0) {
        for (CaseDeformation *deform in deformArray) {
            ...
            //测试
            //NSString *quantity=@"101110111000";
            NSCharacterSet *characterSet1=[NSCharacterSet characterSetWithCharactersInString:@"."];
            NSArray *array = [quantity componentsSeparatedByCharactersInSet:characterSet1];
            
            NSCharacterSet *zeroSet=[NSCharacterSet characterSetWithCharactersInString:@".0"];
            quantity=[array[1] stringByTrimmingTrailingCharactersInSet:zeroSet];
            NSString *quantity2;
            if (quantity.length==0) {
                quantity2=array[0];
            }else{
                quantity2 = [NSString stringWithFormat:@"%@.%@",array[0],quantity];
            }
            deformString=[deformString stringByAppendingFormat:@"、%@%@%@%@%@",deform.roadasset_name,roadSizeString,quantity2,deform.unit,remarkString];
        }
        NSCharacterSet *charSet=[NSCharacterSet characterSetWithCharactersInString:@"、"];
        deformString=[deformString stringByTrimmingCharactersInSet:charSet];
    } else {
...
-(NSString*)generateCommonInquireText{
    ...
    if (orgInfo != nil && orgInfo.belongtoorg_id != nil && ![orgInfo.belongtoorg_id isEmpty]) {
        orgInfo = [OrgInfo orgInfoForOrgID:orgInfo.belongtoorg_id];       //orgInfo表里的belongtoorg_id字段是所属大队的ID
    }
    
    NSString *organizationName = [orgInfo valueForKey:@"orgname"];        //orgInfo表里的orgname字段是机构的全称，orgshortname字段是机构的简称
//      NSString *organizationName =[[OrgInfo orgInfoForOrgID:[UserInfo userInfoForUserID:currentUserID].organization_id] valueForKey:@"orgname"];
    ...
    NSString* text = [NSString stringWithFormat:@"%@%@%@",@"问：你好，我们是",organizationName,@"路政员，向你了解一些情况，希望你如实回答，不得说假话做伪证，否则是要承担法律责任的，你明白吗？\n" ];
