        ...
        [TBXML iterateAttributesOfElement:node withBlock:^(TBXMLAttribute *attribute, NSString *attributeName, NSString *attributeValue) {
            ...
            if ([attributeName isEqualToString:XMLPDFAttributeBorderWidth]) {
                ...
                if ([tagName isEqualToString:XMLPDFTagTable] ||
                    ...
                    if(( [[UIDevice currentDevice] systemVersion].floatValue>=10.0) && [tagName isEqualToString:XMLPDFTagParagraph] && (rect.size.height>20)){
                        rect=CGRectMake(rect.origin.x, rect.origin.y+3, rect.size.width, rect.size.height);
                    }
...
- (void)drawParagraphToContextWithNode:(TBXMLElement *)paragraphNode offset:(CGPoint)offset dataOnly:(BOOL)isDataOnly
{
    ...
    if (!isDataOnly || !isStatic) {
        ...
        CTFramesetterRef framesetter2 = CTFramesetterCreateWithAttributedString((__bridge CFAttributedStringRef)attributedText);
        ...
        CGContextRef context = UIGraphicsGetCurrentContext();
        CGContextSaveGState(context);        CGContextSetTextMatrix(context, CGAffineTransformIdentity);
...
-(float)xxx3:(CGFloat)orig{
    NSInteger n=(NSInteger)(orig*1000+0.5);
