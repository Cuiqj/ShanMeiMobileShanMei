...
//            }
            ...
            caseDescString=[caseDescString stringByAppendingFormat:@"   %@于%@驾驶%@%@行至%@%@%@,在公路%@发生交通事故损坏公路路产，%@，经与当事人现场勘查，",citizen.party,happenDate,citizen.automobile_number,citizen.automobile_pattern,roadName,caseInfo.side,stationString,caseInfo.place,caseStatusString];
            ...
            if (deformArray.count>0) {
                ...
                for (CaseDeformation *deform in deformArray) {
                    ...
                    if ([remarkString isEmpty]) {
                     ...
                     NSString * quantity= [NSString stringWithFormat:@"%.2f",deform.quantity.floatValue];
...
//                    quantity=[quantity stringByTrimmingTrailingCharactersInSet:zeroSet];
                    deformsString=[deformsString stringByAppendingFormat:@"、%@%@%@%@%@",deform.roadasset_name,roadSizeString,deform.quantity,deform.unit,remarkString];
                }
...
//                caseStatusString=[caseStatusString stringByAppendingString:@"未造成车辆损坏"];
//            }
            caseDescString=[caseDescString stringByAppendingFormat:@"在公路%@发生交通事故损坏公路路产，%@，经与当事人现场勘查，",caseInfo.place,caseStatusString];
...
//            caseDescString = [caseDescString stringByAppendingFormat:@"我于%@驾驶%@%@行至%@%@%@，发生交通事故，造成了路产损坏。",happenDate,citizen.automobile_number,citizen.automobile_pattern,roadName,caseInfo.side,stationString];
            caseDescString = [caseDescString stringByAppendingFormat:@"我于%@驾驶%@%@行至%@%@%@由于%@发生交通事故，造成了路产损坏。",happenDate,citizen.automobile_number,citizen.automobile_pattern,roadName,caseInfo.side,stationString,caseInfo.case_reason];
...
+ (NSString*)generateDefaultPayReason:(NSString *)caseID{
    ...
    if (matchLaws) {
        ...
        payReason = [NSString stringWithFormat:@"你违反了%@，根据%@，并依照%@文件的规定，你损坏公路路产，应照价赔偿路产损失，你有无异议？",breakStr, matchStr, payStr];
...
//            }
//            
            caseDescString=[caseDescString stringByAppendingFormat:@"%@于%@驾驶%@%@行至%@%@%@,在公路%@发生交通事故损坏公路路产，%@。",citizen.party,happenDate,citizen.automobile_number,citizen.automobile_pattern,roadName,caseInfo.side,stationString,caseInfo.place,caseStatusString];
        }
        if (citizenArray.count>1) {
            Citizen *citizen=[citizenArray objectAtIndex:0];
