...
- (void)viewDidLoad
{
    ...
    if (self.docPrinterState==kPDFView) {
        ...
        if ([[self.TableMapping objectForKey:self.fileName] isEqualToString:@"ProveInfoTable"] ||
            ...
            [[self.TableMapping objectForKey:self.fileName] isEqualToString:@"CancelParkingNodeTable"] ||
...
- (IBAction)btnPrintPreview:(id)sender {
    if (self.pageControl.currentPage == 1) {
        ...
        [UIView animateWithDuration:0.5
                         ...
                         completion:nil];
        self.pdfFileURL=[self.docPrinter toFullPDFWithPath:[self docPathFromFileName]];   //去填充
...
-(NSString *)docPathFromFileName{
    if (![self.caseID isEmpty] || [self.fileName isEqualToString:@"施工整改通知书"]) {
        ...
        NSLog(@"施工整改通知书 == %@",fileName);  //赔（补）偿清单-0.pdf
        NSArray *arrayPaths=NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
        NSString *path=[arrayPaths objectAtIndex:0];
        NSLog(@"施工整改 ==  %@",path);
        return [path stringByAppendingPathComponent:fileName];    //在沙盒里面找
    } else {
        return @"";
    }
}
