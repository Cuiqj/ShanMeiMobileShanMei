...
- (void)viewDidLoad{
    ...
    NSLog(@"1显示的为:%@",casemarkPaddingView.text);
...
//案号年份自动生成
    ...
    NSString *tempmonthString=[dateString substringToIndex:7];
    NSString *monthString =[tempmonthString substringFromIndex:5];
...
//    NSString *caseMark3 = [[NSString alloc] initWithFormat:@"%ld",MAX(caseMark3InDefaults, caseMark3InCoreData)+1];
    ...
    // 月份小于十的前面添0
//    if (caseMark3.length<5) {
//        NSString *firstNO=@"0";
//        caseMark3=[NSString stringWithFormat:@"%@%@",firstNO,caseMark3];
//    }
//    NSString *cuttomonthString=[[NSUserDefaults standardUserDefaults] stringForKey:@"CaseMark3"];
//    NSString *tempoldmonthString=[cuttomonthString substringToIndex:3];
//    NSString *oldmonthString=[tempoldmonthString substringFromIndex:1];
...
//    }
//    if (monthString.intValue>oldmonthString.intValue) {
//        caseMark3=[[NSString alloc]initWithFormat:@"%@001",monthString];
//     
//    }
...
//    [[NSUserDefaults standardUserDefaults] synchronize];
    ...
    NSLog(@"2显示的为:%@",self.textCasemark3.text);
    
    //删除无用的案件数据
    [CaseInfo deleteEmptyCaseInfo];
    
    //分配必要的tag
    self.textWeatheer.tag = kUITextFieldTagWeather;                     //通过tag找到视图
    self.docListView.tag = kUITableViewTagDocGenerated;                 //通过tag找到视图
    self.docTemplatesView.tag = kUITableViewTagDocTemplate;             //通过tag找到视图
    ...
    [self.segInfoPage setTitleTextAttributes:attributes
                                    forState:UIControlStateNormal];          //frame = (80 262; 540 30);
     NSLog(@"3显示的为:%@",self.segInfoPage);
...
//    if (monthString.intValue>oldmonthString.intValue) {
//        caseMark3=[[NSString alloc]initWithFormat:@"%@001",monthString];
//        [[NSUserDefaults standardUserDefaults] setObject:yearString forKey:@"CaseMark3"];
//    }
...
- (void)loadCaseInfoForCase:(NSString *)caseID{
    ...
    if (self.caseInfo) {
        ...
        [formatter setLocale:[NSLocale currentLocale]]; //案发地点
        [formatter setDateFormat:@"yyyy-MM-dd HH:mm"];//转换格式
    ...
    [self loadCitizenInfoForCase:caseID andAutoNumber:@""];  //根据案件号和车牌号载入车辆及车主信息
    [self loadCaseProveInfoForCase:caseID];                  //根据caseID载入勘验信息
    [self loadCaseDocList:caseID];                          //载入案件已生成文书信息
    [self loadCasePhotoForCase:caseID];                     //载入案件对应的照片
    [self.accInfoBriefVC loadDataForCase:caseID];           //根据caseID载入相应案件数据
}
...
- (void)saveCaseInfoForCase:(NSString *)caseID{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:self.caseInfo.case_mark2 forKey:@"CaseMark2"];
    NSArray *array = [self.textCasemark3.text componentsSeparatedByString:@"-"];
    if ([array count] >= 2 ) {
        self.caseInfo.case_mark3=[array objectAtIndex:1];
        NSString *caseMark3=[[NSString alloc]initWithFormat:@"%@",self.caseInfo.case_mark3];
        [[NSUserDefaults standardUserDefaults] setObject:caseMark3 forKey:@"CaseMark3"];
    }else {
...
- (void)setCaseIDdelegate2:(NSString *)caseID{
    [self loadCaseInfoForCase:caseID];  //根据caseID载入相应案件数据
    if (self.segInfoPage.selectedSegmentIndex!=0) {    //判断是否有案件
        for (UIView *view in self.infoView.subviews) {
            [view removeFromSuperview];
        }
        [self.labelPhotoIndex setAlpha:0.0];
        ...
        [self.segInfoPage setSelectedSegmentIndex:0];    //重置判断是否有案件
    }
}
